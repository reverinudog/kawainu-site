...
- [ ] **お祓いシミュレーター iPhoneノッチ対応** — `#overlay`の`padding-top: max(20px, env(safe-area-inset-top) + 56px)` が効いていない（競合調査必要）。タイトル・説明・転がすボタンが上すぎてiPhoneのステータスバー/ノッチと被る
- [ ] **お祓いデバッグパネル除去・developマージ** — safe-area修正完了後、`feature/debug-result-check`からデバッグパネル常時表示を元に戻して(`?debug`クエリ方式に復元)、developへマージ
- [ ] **アバター画像の差し替え**
- [ ] **将来的: 独自ドメイン取得**
