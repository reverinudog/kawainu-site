...
- [ ] **ブート演出の微調整** — ズームイン完了→実サイト遷移時に微妙に左上にずれる問題を修正。transform-origin の逆算をピクセル単位で調整
- [ ] **Achievements セクション改修** — 最新2年分のみ初期表示、それ以前は「以前の歩みを見る」ボタンで展開。ナビバードロップダウンも最新2年のみ表示（ALL リンク削除）
