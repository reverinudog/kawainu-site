...
- [ ] **お祓いシミュレーター ダイス結果パネル全行表示** — iPhone縦画面で `results-panel` が8行目までしか表示されない。スクロール可能にするか、グリッドサイズを調整して全10行見えるようにする。テスト時はiPhone比率（390×844等）を使用すること
- [x] **お祓いシミュレーター iPhoneノッチ対応** — モバイルメディアクエリのsafe-area上書き修正済み
- [x] **お祓いデバッグパネル除去・developマージ** — `?debug=1`クエリ方式に復元、develop・masterマージ済み
- [ ] **アバター画像の差し替え**
- [ ] **将来的: 独自ドメイン取得**
