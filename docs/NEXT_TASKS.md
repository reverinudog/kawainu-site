...
- [x] **ナビバー モバイルレイアウト修正** — 768px以下でハンバーガーメニュー化（アコーディオンドロップダウン対応）
- [x] **お祓いシミュレーター 広告導入** — ゲームページに広告を配置（AdSense、インタースティシャル、プレースホルダーID）
- [x] **Contact セクション文言変更** — 「お気軽にご連絡ください」→「お問い合わせはこちらから」に変更
- [ ] **お問い合わせフォーム メール送信機能** — 送信ボタンで実際にメールが飛ぶように（Cloudflare Workers + 外部メールAPI or Formspree）
- [ ] **AdSense 広告スロットID差し替え** — 審査承認後、`ZZZZZZZZZ` を実際のスロットIDに置換
- [ ] **アバター画像の差し替え**
- [ ] **将来的: 独自ドメイン取得**
...
- [x] お祓いシミュレーター スマホ縦画面修正 — クリティカル/ファンブル枠の2列問題（flex-wrap競合解消）、リザルトメッセージのtranslateY(-50%)除去・全演出でモバイル対応を適用
- [x] お祓いシミュレーター リザルト動的配置 — `getBoundingClientRect()`でボタン下端・ダイスグリッド上端を取得し、メッセージ・statsを動的配置。ボタン/メッセージ/stats/ダイスの重なり解消
- [x] お祓いシミュレーター 全リザルトフォントサイズ統一 — 爆発メッセージ(3rem)含む全演出で`applyMobileMsgStyle`適用
- [x] お祓いシミュレーター ダイス結果パネル全行表示 — `aspect-ratio: auto`でセル高さ縮小、`overflowY: hidden`で48vh内にスクロールなし全10行表示
- [x] お祓いシミュレーター リザルト表示デスクトップ/モバイル両対応 — `translateY(-50%)`除去+全サイズ動的配置、絵文字スケール大幅縮小(0.22-0.30)、個数半減、配置範囲制限、stats z-index:110
- [x] お祓いシミュレーター スマホ盤面リサイズ — `updateCameraForViewport()` でアスペクト比に応じたFOV/カメラY/Z動的調整（モバイル: FOV65/Y70/Z50 ↔ デスク: FOV55/Y55/Z35、線形補間）
